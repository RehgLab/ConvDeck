...
@tenacity
def ppt_to_images(file: str, output_dir: str, warning: bool = False, dpi=72, output_type='png', file_name: str = "poster"):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for f in os.listdir(temp_dir):
            ...
            for i, img in enumerate(images):
                if output_type == 'png':
                    img.save(pjoin(output_dir, f"{file_name}.png"), 'PNG')
                else:
                    img.save(pjoin(output_dir, f"{file_name}.jpg"), 'JPEG')
            return
...
_AUTHOR_YEAR_RE = _re.compile(
    r"[A-Z][A-Za-z\-’']+(?:\s+(?:et\s+al\.?|and|&)\s+[A-Z][A-Za-z\-’']+)*"
    r"(?:'s)?[,\s]*\(?\s*(?:19|20)\d{2}[a-z]?\)?"
)
_NUMERIC_RE = _re.compile(r"\[\s*\d+(?:\s*[\-–,;]\s*\d+)*\s*\]")


def _expand_numeric_citation(token):
    """[2-4] -> ['[2]','[3]','[4]']; [1,3] -> ['[1]','[3]']."""
    inner = token.strip("[] ")
    nums = []
...
def _normalize_citation(s):
    return _re.sub(r"\s+", " ", s.replace("&", "and")).lower().strip(" .,;:()")


def split_llm_references(ref_str):
    """Split an LLM-emitted 'reference' string into individual citation tokens.

    Handles both numeric ([1], [2,3], [4-6]) and author-year (Smith et al., 2020)
    formats; deduplicates while preserving order.
    """
    if not ref_str:
        return []
    tokens = [m.group(0) for m in _NUMERIC_RE.finditer(ref_str)]
    leftover = _NUMERIC_RE.sub(" ", ref_str)
    for chunk in _re.split(r"\s*;\s*", leftover):
        chunk = chunk.strip(" .,()")
        if not chunk:
            continue
        for m in _AUTHOR_YEAR_RE.finditer(chunk):
            tokens.append(m.group(0))
    seen, uniq = set(), []
    for t in tokens:
        key = _normalize_citation(t)
...
def collect_raw_content_corpus(raw_json):
    """Concatenate every subsection 'content' string from a raw_content.json."""
    parts = []
...
def find_subsection_content(raw_json, section_title, subsection_title):
    section_norm = _normalize_citation(section_title or "")
    sub_norm = _normalize_citation(subsection_title or "")
    for section in raw_json.get("sections", []):
        if section_title and _normalize_citation(section.get("title", "")) != section_norm:
            continue
        for sub in section.get("subsections", []):
            if not subsection_title or _normalize_citation(sub.get("title", "")) == sub_norm:
                return sub.get("content", "") or ""
    for section in raw_json.get("sections", []):
        for sub in section.get("subsections", []):
            if subsection_title and _normalize_citation(sub.get("title", "")) == sub_norm:
                return sub.get("content", "") or ""
    return ""


def citation_in_text(token, text):
    """True if `token` (numeric like '[3]' or author-year) appears in `text`."""
    if not text:
        return False
    if _NUMERIC_RE.fullmatch(token):
        atoms = _expand_numeric_citation(token)
        norm_text = _normalize_citation(text)
        return all(atom in norm_text or atom.replace(" ", "") in norm_text
                   for atom in atoms)
    norm_token = _normalize_citation(token)
    norm_text = _normalize_citation(text)
    if norm_token in norm_text:
        return True
    m = _re.search(r"([A-Z][A-Za-z\-’']+)", token)
    year_m = _re.search(r"(19|20)\d{2}", token)
    if m and year_m:
        author = _normalize_citation(m.group(1))
        year = year_m.group(0)
        for hit in _re.finditer(_re.escape(author), norm_text):
            if year in norm_text[hit.end(): hit.end() + 60]:
                return True
    return False


def validate_slide_references(slide_plan, raw_json):
    """Drop any citation in slide_plan['slides'][*]['reference'] that doesn't
    appear in the corresponding subsection content (falling back to the full
    paper corpus). Mutates and returns slide_plan."""
    corpus = collect_raw_content_corpus(raw_json)
    for slide in slide_plan.get("slides", []):
        tokens = split_llm_references(slide.get("reference", "") or "")
        if not tokens:
            slide["reference"] = ""
            continue
        sub_content = find_subsection_content(
            raw_json, slide.get("section", ""), slide.get("subsection", "")
        )
        kept = [t for t in tokens
                if citation_in_text(t, sub_content) or citation_in_text(t, corpus)]
        slide["reference"] = "; ".join(kept)
    return slide_plan
